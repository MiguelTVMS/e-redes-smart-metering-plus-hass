...
Write-Host "Locating Python 3.13..."
...
if ($LASTEXITCODE -ne 0) {
    ...
    if ($LASTEXITCODE -ne 0) {
        & uv python find 3.13 *> $null
        if ($LASTEXITCODE -ne 0) {
            throw "uv could not install or locate Python 3.13."
        }

        Write-Warning "uv reported an installation-link error, but Python 3.13 is available and will be used."
    }
}
...
if (Test-Path $venvPython) {
    & $venvPython -c "import sys; raise SystemExit(sys.version_info[:2] != (3, 13))"
    $reuseVenv = $LASTEXITCODE -eq 0
}

if ($reuseVenv) {
    Write-Host "Reusing the existing Python 3.13 environment..."
}
else {
    Write-Host "Creating the local Python environment..."
    Invoke-NativeCommand -Command { uv venv --clear --python 3.13 .venv } `
        -FailureMessage "Failed to create the local Python 3.13 environment."
}
